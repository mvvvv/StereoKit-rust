...
impl Layers1 {
    ...
    fn draw(&mut self, token: &MainThreadToken) {
        ...
        Ui::label(format!("Sort Order {}", self.quad_sort_order as i32), Some(Vec2::new(0.1, 0.0)), false);
        Ui::same_line();
        Ui::hslider("Sort Order", &mut self.quad_sort_order, -1.0, 1.0, Some(1.0), None, None, None);

        Ui::hseparator();
        Ui::label("Cylinder Layer", None, false);
        Ui::label(format!("Sort Order {}", self.cylinder_sort_order as i32), Some(Vec2::new(0.1, 0.0)), false);
        Ui::same_line();
        Ui::hslider("Cyl Sort Order", &mut self.cylinder_sort_order, -1.0, 1.0, Some(1.0), None, None, None);
        Ui::label(format!("Radius {:.2}m", self.cylinder_radius), Some(Vec2::new(0.1, 0.0)), false);
        Ui::same_line();
        Ui::hslider("Cyl Radius", &mut self.cylinder_radius, 0.2, 3.0, None, None, None, None);
        Ui::label(format!("Arc {:.0}°", self.cylinder_angle.to_degrees()), Some(Vec2::new(0.1, 0.0)), false);
        Ui::same_line();
        Ui::hslider("Cyl Angle", &mut self.cylinder_angle, 0.1, std::f32::consts::TAU, None, None, None, None);
        Ui::label(format!("Aspect {:.2}", self.cylinder_aspect), Some(Vec2::new(0.1, 0.0)), false);
        Ui::same_line();
