...
impl Default for Font1 {
    fn default() -> Self {
        ...
        let emoji_style = Text::make_style(emoji_font, 0.35, BLUE);
...
impl Font1 {
    ...
    fn start(&mut self) -> bool {
        self.title_style = Text::make_style(Font::default(), 0.3, RED);
        true
    }
    ...
    fn draw(&mut self, _token: &MainThreadToken) {
        TextBuilder::new("😋 Emojis🤪\n\n  🧐").transform(self.transform).style(self.emoji_style).add();

        Ui::window("Default Font").pose(&mut self.window_pose).begin();
        Ui::push_text_style(self.text_style);
        Ui::text("text font").size([0.14, 0.0]).draw();
        Ui::pop_text_style();
        Ui::window_end();

        TextBuilder::new(&self.text).transform(self.transform).style(self.title_style).add();
    }
}
