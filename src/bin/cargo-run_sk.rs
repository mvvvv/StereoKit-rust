//! `cargo-run_sk` — a real-time viewer with in-process hot-reload for StereoKit-rust projects (Linux).
//!
...
//!
//! The host and the plugin both link the same shared `libStereoKitC.so` (feature `skc-shared`), so there is exactly
//! ONE StereoKit engine instance in the process: the plugin drives the host's session through it.
//!
...
#[cfg(not(any(target_os = "linux", feature = "no-event-loop")))]
fn main() {
    eprintln!("cargo-run_sk: only supported on Linux for now");
}

#[cfg(all(target_os = "linux", feature = "no-event-loop"))]
fn main() {
    eprintln!("cargo-run_sk: requires the default event loop framework (do not use the no-event-loop feature)");
}

#[cfg(all(target_os = "linux", not(feature = "no-event-loop")))]
fn main() {
    imp::main();
}
...
mod imp {
    ...
    plugin library each time it is rebuilt.
        ...
        --lib <path>           : plugin library to load
                                 ...
                                 src/bin/main_<crate>.rs -> target/debug/lib<crate>.so,
                                 examples/main*.rs -> target/debug/examples/lib<name>.so,
                                 else the first .so under target/debug/examples)
        --start <view name>    : view to select at start (name, or substring)
        --watch <secs>         : plugin .so watch period, 0 disables (default 0.5)
    ...
    struct Plugin {
        /// The dlopen'ed library: kept alive exactly as long as the fn
    ...
    /// Copies `src` to a unique file then dlopens it, checks the ABI and crate
    ...
    fn load_plugin(sk: Option<*mut c_void>, src: &Path, copy_counter: &mut u32) -> Result<Plugin, String> {
        // Copy the freshly built .so to a unique name: cargo can then replace
        // the original file while we keep the previous plugin loaded.
        let copy_dir = Path::new("target/run_sk");
        fs::create_dir_all(copy_dir).map_err(|e| format!("cannot create {}: {e}", copy_dir.display()))?;
        let copy = copy_dir.join(format!("plugin-{copy_counter}.so"));
        *copy_counter += 1;
        ...
        unsafe {
            let lib = Library::new(&copy).map_err(|e| format!("dlopen {} failed: {e}", copy.display()))?;
    ...
    ///    its plugin is `target/debug/lib<crate>.so` (e.g. `libsk_project.so` for
    ///    the project `sk_project`),
    /// 2. `examples/main*.rs` exists -> the StereoKit-rust repository itself, its
    ///    plugin is `target/debug/examples/lib<name>.so` (e.g. `libmain_pc.so` for
    ///    `examples/main_pc.rs`),
    /// 3. fallback: the first `.so` under `target/debug/examples/` (e.g. cargo
    ///    hashed copies), then the historical `target/debug/examples/libmain.so`.
    fn default_lib_path() -> PathBuf {
        ...
        if let Ok(name) = get_cargo_name() {
            ...
            if PathBuf::from(format!("src/bin/main_{lib_name}.rs")).exists() {
                return PathBuf::from(format!("target/debug/lib{lib_name}.so"));
            }
        }
        // 2 - the StereoKit-rust repository itself
        if let Some(example) = find_example_source_name() {
            return PathBuf::from(format!("target/debug/examples/lib{example}.so"));
        }
        ...
        PathBuf::from("target/debug/examples/libmain.so")
    }
    ...
    /// Runs the build command once (`sh -c <cmd>`) and reports the outcome to the
    /// main loop. Build errors are reported but never fatal: the dev viewer keeps
    /// the last working plugin loaded.
    fn run_build(build_cmd: &str, sender: &Sender<HostMsg>) {
        sender.send(HostMsg::BuildStarted).ok();
        match Command::new("sh").arg("-c").arg(&build_cmd).output() {
            Ok(output) => {
    ...
    /// command (`sh -c <cmd>`). The build (derived from the project layout, or the
